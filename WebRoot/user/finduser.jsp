...
<hr />
 <form  method="post" action="act_User_updateUser?id=${bean.id}" class="demoform">
  <p style="color: red;font-size: 16px;">${message}</p>
    <table width="400" border="1"   bordercolor="#3399FF" id="customers">
      ...
      <tr>
	   ...
	   <div>
	      ...
	      <c:forEach items="${roleList}" var="bean2">
	      	<option value="${bean2.dicscode}" <c:if test="${bean.role.dicscode eq bean2.dicscode}">selected="selected"</c:if>>${bean2.dicname}</option>
	      </c:forEach>
	      </select>
		</div>
       </td>
      ...
      <tr>
	   <th width="50%" height="30" align="center" bgcolor="#3399ff"><span class="need" style="width:10px;">*</span>旧密码:</th>
	   <td>
	   <div><input type="password" name="oldpassword" class="inputxt" datatype="*6-16" nullmsg="请设置密码！" errormsg="密码范围在6~16位之间！"/></div>
       </td>
	  </tr>
	  	  
      <tr>
	   <th width="50%" height="30" align="center" bgcolor="#3399ff"><span class="need" style="width:10px;">*</span>新密码:</th>
	   <td>
      ...
      <tr>
	   <th width="50%" height="30" align="center" bgcolor="#3399ff"><span class="need" style="width:10px;">*</span>确认新密码:</th>
	   <td>
