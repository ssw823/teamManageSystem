...
<span class="title">
<c:if test="${status==0}">修改球队事物公告</c:if>
<c:if test="${status==1}">修改经理人公告</c:if>
<c:if test="${status==2}">修改规则知识</c:if>
<c:if test="${status==3}">修改球队赛事记录</c:if>	
</span>
<hr />
 <form  method="post" action="act_News_updateNews?status=${status}&id=${bean.id}" class="demoform">
  <p style="color: red;font-size: 16px;">${message}</p>
    <table width="400" border="1"   bordercolor="#3399FF" id="customers">
      ...
      <tr>
	   ...
	   <td>
	   <div><input type="text" value="${bean.title}" name="title" class="inputxt" datatype="*" errormsg="请输入标题！" /></div>
       </td>
	  </tr>
